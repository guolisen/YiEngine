...
static double EL3[27] = {2892,5.8438,6283.0758,168,5.488,12566.152,30,5.2,155.42,13,4.72,3.52,7,5.3,18849.23,6,5.97,242.73,4,3.79,553.57,1,4.3,6286.6,1,0.91,6127.66};//EL3
static double EL4[9]  = {77,4.13,6283.08,8,3.84,12566.15,4,0.42,155.42};//EL4
static double EL5[6]  = {2,2.77,6283.08,1,2.01,155.42};//EL5


/// <summary>
/// 地球黄纬数据,误差0.2"
/// </summary>
static double EB0[18] = 
{2796,3.1987,84334.6616,1016,5.4225,5507.5532,804,3.88,5223.694,438,3.704,2352.866,319,4,1577.344,227,3.985,1047.747};//EB0
static double EB1[6] =    
...
static double ER1[9] = 
    {1030186,1.1074897,6283.07585,17212,1.06442,12566.1517,7022,3.1416,0};//ER1
static double ER2[12] =
    {43594,5.78455,6283.07585,1236,5.5793,12566.1517,123,3.142,0,88,3.63,77713.77};//ER2
static double ER3[6] =
...
static double MR2[24] = 
{//MR2
    0.0015,4.2,14914.452,-1,6,0,0.0011,1.8,6585.761,-2,-19,0,
    0.0009,2.4,7700.389,2,25,0,0.0008,5.5,8956.993,1,25,0
};

StarCalendar::StarCalendar(void)
{
    int i = 0;
    for(i = 0; i < 363; ++i)
        EL_[0].push_back(EL0[i]);
    for(i = 0; i < 240; ++i)
        EL_[1].push_back(EL1[i]);
    for(i = 0; i < 168; ++i)
        EL_[2].push_back(EL2[i]);
    for(i = 0; i < 27; ++i)
        EL_[3].push_back(EL3[i]);
    for(i = 0; i < 9; ++i)
        EL_[4].push_back(EL4[i]);
    for(i = 0; i < 6; ++i)
        EL_[5].push_back(EL5[i]);

    for(i = 0; i < 18; ++i)
        EB_[0].push_back(EB0[i]);
    for(i = 0; i < 6; ++i)
        EB_[1].push_back(EB1[i]);
    for(i = 0; i < 3; ++i)
        EB_[2].push_back(EB2[i]);

    for(i = 0; i < 48; ++i)
        ER_[0].push_back(ER0[i]);
    for(i = 0; i < 9; ++i)
        ER_[1].push_back(ER1[i]);
    for(i = 0; i < 12; ++i)
        ER_[2].push_back(ER2[i]);
    for(i = 0; i < 6; ++i)
        ER_[3].push_back(ER3[i]);
    for(i = 0; i < 6; ++i)
        ER_[4].push_back(ER4[i]);
    for(i = 0; i < 3; ++i)
        ER_[5].push_back(ER5[i]);

    for(i = 0; i < 954; ++i)
        ML_[0].push_back(ML0[i]);
    for(i = 0; i < 198; ++i)
        ML_[1].push_back(ML1[i]);
    for(i = 0; i < 36; ++i)
        ML_[2].push_back(ML2[i]);

    for(i = 0; i < 516; ++i)
        MB_[0].push_back(MB0[i]);
    for(i = 0; i < 84; ++i)
        MB_[1].push_back(MB1[i]);

    for(i = 0; i < 522; ++i)
        MR_[0].push_back(MR0[i]);
    for(i = 0; i < 120; ++i)
        MR_[1].push_back(MR1[i]);
    for(i = 0; i < 24; ++i)
        MR_[2].push_back(MR2[i]);
}
StarCalendar::~StarCalendar(void)
{
}
...
/// <param name="n"></param>
/// <returns></returns>
double StarCalendar::Enn(std::vector<double> ob[], int lineNum, XL::E_TYPE type, double t, double n)
{
    int i, j;
    std::vector<double> F;
    double N, v = 0, tn = 1, c;
    if (type == XL::EL_TYPE)
    {
        ...
        v += 1753469512 + 6283319653318 * t + 529674 * t2 + 432 * t3 - 1124 * t4 - 9 * t5 + 630 * cos(6 + 3 * t); //地球平黄经(已拟合DE406)
    }
    n *= 3; if (n < 0) n = ob[0].size();
    for (i = 0; i < lineNum; i++)  // C#: 注释循环变量中步长计算的后半语句:   , tn *= t)
    {
        F = ob[i];
        N = floor(n * F.size() / ob[0].size() + 0.5); if (i != 0) N += 3; if (N >= F.size()) N = F.size();
        for (j = 0, c = 0; j < N; j += 3) c += F[j] * cos(F[j + 1] + t * F[j + 2]);
        // v += c * tn;     // C#: 注释此句并改写如下
        v += c * tn * pow(t, i);
    }
    return v / 1000000000;
...
/// <param name="n2"></param>
/// <param name="n3"></param>
void StarCalendar::E_coord(double t, double re[4], double n1, double n2, double n3)
{  
    t /= 10;
    re[0] = Enn(EL_, 6, XL::EL_TYPE, t, n1);
    re[1] = Enn(EB_, 3, XL::EB_TYPE, t, n2);
    re[2] = Enn(ER_, 6, XL::ER_TYPE, t, n3);
}
...
/// <param name="n"></param>
/// <returns></returns>
double StarCalendar::E_Lon(double t, double n)
{
    return Enn(EL_, 6, XL::EL_TYPE, t / 10, n);
}
...
/// <param name="n"></param>
/// <returns></returns>
double StarCalendar::Mnn(std::vector<double> ob[], int lineNum, XL::M_TYPE type, double t, double n)
{
    int i, j;
    std::vector<double> F;
    double N, v = 0, tn = 1, c;
    ...
    if (type == XL::ML_TYPE)
    {
        v += (3.81034409 + 8399.684730072 * t - 3.319e-05 * t2 + 3.11e-08 * t3 - 2.033e-10 * t4) * commonDef::RAD; //月球平黄经(弧度)
    ...
    {
        F = ob[i];
        N = floor(n * F.size() / ob[0].size() + 0.5); if (i!=0) N += 6; if (N >= F.size()) N = F.size();
        for (j = 0, c = 0; j < N; j += 6)
        {
            // c += F[j] * cos(F[j + 1] + t * F[j + 2] + t2 * F[j + 3] + t3 * F[j + 4] + t4 * F[j + 5]);  // C#: 原语句, 因可能导致数据精度过低, 故修改如下
            c += F[j] * cos(F[j + 1] + t * F[j + 2] + t2 * F[j + 3] / 1e4 + t3 * F[j + 4] / 1e8 + t4 * F[j + 5] / 1e8);  // C#: 新语句
...
/// <param name="n2"></param>
/// <param name="n3"></param>
void StarCalendar::M_coord(double t, double re[4], double n1, double n2, double n3)
{
    re[0] = Mnn(ML_, 6, XL::ML_TYPE, t, n1);
    re[1] = Mnn(MB_, 2, XL::MB_TYPE, t, n2);
    re[2] = Mnn(MR_, 3, XL::MR_TYPE, t, n3);
}
...
/// <param name="n2"></param>
/// <param name="n3"></param>
void M_coord(double t, LunarInfoListT<double> re, double n1, double n2, double n3)    // C#: 新扩展出来的方法
{
    re[0] = Mnn(ML, t, n1);
    re[1] = Mnn(MB, t, n2);
    re[2] = Mnn(MR, t, n3);
...
/// <param name="n"></param>
/// <returns></returns>
double StarCalendar::M_Lon(double t, double n)
{
    return Mnn(ML_, 6, XL::ML_TYPE, t, n);
}
...
/// <param name="t"></param>
/// <returns></returns>
double StarCalendar::E_v(double t)
{
...
/// <param name="t"></param>
/// <returns></returns>
double StarCalendar::M_v(double t)
{
...
/// <param name="Sn"></param>
/// <returns></returns>
double StarCalendar::MS_aLon(double t, double Mn, double Sn)
{
    return 0.0;
    return M_Lon(t, Mn) + coordinate_.gxc_moonLon(t) - (E_Lon(t, Sn) + coordinate_.gxc_sunLon(t) + M_PI);
}
...
/// <param name="n"></param>
/// <returns></returns>
double StarCalendar::S_aLon(double t, double n)
{
    //return 0.0;
    return E_Lon(t, n) + coordinate_.nutationLon(t) + coordinate_.gxc_sunLon(t) + M_PI; //注意，这里的章动计算很耗时
}
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::E_Lon_t(double W)
{
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::M_Lon_t(double W)
{
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::MS_aLon_t(double W)
{
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::S_aLon_t(double W)
{
    double t, v = 628.3319653318;
    t = (W - 1.75347 - M_PI) / v; v = E_v(t); //v的精度0.03%，详见原文
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::MS_aLon_t2(double W)
{ 
    double t, v = 7771.37714500204;
    t = (W + 1.08472) / v;
    double L, t2 = t * t;
    ...
    L = M_Lon(t, 20) - (4.8950632 + 628.3319653318 * t + 0.000005297 * t * t + 0.0334166 * cos(4.669257 + 628.307585 * t) + 0.0002061 * cos(2.67823 + 628.307585 * t) * t + 0.000349 * cos(4.6261 + 1256.61517 * t) - 20.5 / commonDef::RAD);
...
/// <param name="W"></param>
/// <returns></returns>
double StarCalendar::S_aLon_t2(double W)
{
    double t, v = 628.3319653318;
    t = (W - 1.75347 - M_PI) / v;
    t -= (0.000005297 * t * t + 0.0334166 * cos(4.669257 + 628.307585 * t) + 0.0002061 * cos(2.67823 + 628.307585 * t) * t) / v;
    t += (W - E_Lon(t, 8) - M_PI + (20.5 + 17.2 * sin(2.1824 - 33.75705 * t)) / commonDef::RAD) / v;
    return t;
}
...
/// <param name="t"></param>
/// <returns></returns>
double StarCalendar::moonIll(double t)
{
    double t2 = t * t, t3 = t2 * t, t4 = t3 * t;
    double D, M, m, a, dm = M_PI / 180;
    ...
    D *= dm; M *= dm; m *= dm;
    a = M_PI - D + (-6.289 * sin(m) + 2.100 * sin(M) - 1.274 * sin(D * 2 - m) - 0.658 * sin(D * 2) - 0.214 * sin(m * 2) - 0.110 * sin(D)) * dm;
    return (1 + cos(a)) / 2;
}
...
/// <param name="h"></param>
/// <returns></returns>
double StarCalendar::moonRad(double r, double h)
{ 
    return 358473400 / r * (1 + sin(h) * commonDef::cs_rEar / r);
}
...
/// <param name="t"></param>
/// <returns></returns>
double StarCalendar::shiCha(double t)
{
    double t2 = t * t, t3 = t2 * t, t4 = t3 * t, t5 = t4 * t;
    double L = (1753469512 + 628331965331.8 * t + 5296.74 * t2 + 0.432 * t3 - 0.1124 * t4 - 0.00009 * t5 + 630 * cos(6 + 0.3 * t)) / 1000000000 + M_PI - 20.5 / commonDef::RAD;

    double E, dE, dL;
    double z[2] = {0};    // C#: 待定(?)
    dL = -17.2 * sin(2.1824 - 33.75705 * t) / commonDef::RAD; //黄经章
    dE = 9.2 * cos(2.1824 - 33.75705 * t) / commonDef::RAD; //交角章
    E = coordinate_.hcjj(t) + dE; //真黄赤交角

    //地球坐标
    z[0] = E_Lon(t, 50) + M_PI + coordinate_.gxc_sunLon(t) + dL;
    z[1] = -(2796 * cos(3.1987 + 8433.46616 * t) + 1016 * cos(5.4225 + 550.75532 * t) + 804 * cos(3.88 + 522.3694 * t)) / 1000000000;

    coordinate_.llrConv(z, E); //z太阳地心赤道坐标
    z[0] -= dL * cos(E);

    L = commonDef::rad2mrad(L - z[0]);
    if (L > M_PI) L -= commonDef::pi2;
        return L / commonDef::pi2; //单位是周(天)
...
/// <param name="t"></param>
/// <returns></returns>
double StarCalendar::shiCha2(double t)
{
    double L = (1753469512 + 628331965331.8 * t + 5296.74 * t * t) / 1000000000 + M_PI;
    double z[2] = {0};    // C#: 待定(?)
    double E = (84381.4088 - 46.836051 * t) / commonDef::RAD;
    z[0] = E_Lon(t, 5) + M_PI; z[1] = 0;     // 地球坐标
    coordinate_.llrConv(z, E);     // z太阳地心赤道坐标
    L = commonDef::rad2mrad(L - z[0]);
    if (L > M_PI) 
        L -= commonDef::pi2;
